...
export async function GET(request: Request) {
  ...
  const records = await getRecords({
    action: action && STOCK_ACTIONS.includes(action as (typeof STOCK_ACTIONS)[number])
      ? (action as (typeof STOCK_ACTIONS)[number])
      : "ALL",
    warehouseId: url.searchParams.get("warehouseId") || "ALL",
    status: status && ORDER_STATUSES.includes(status as (typeof ORDER_STATUSES)[number])
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "IN") {
      if (!warehouseId) {
        throw new Error("入库时必须提供 warehouseId");
      }
